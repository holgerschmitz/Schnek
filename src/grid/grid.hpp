...
namespace schnek {
  ...
  namespace internal {
    ...
    class GridBase {
      ...
      protected:
        template<typename ExecutionSpace, typename GridType, typename ExecutionViewType>
        ExecutionViewType getExecutionViewImpl(GridType& grid) {
          if constexpr (std::is_same_v<ExecutionViewType, GridType>) {
            return grid;
          } else {
            return grid.storage.getExecutionView(grid);
          }
        }
    };

    template<typename FallBack, typename ExecutionSpace, typename T, typename = void>
    struct ExecutionViewTypeOr {
        using type = FallBack;
    };

    template<typename FallBack, typename ExecutionSpace, typename T>
    struct ExecutionViewTypeOr<FallBack, ExecutionSpace, T, std::void_t<typename T::template ExecutionViewType<ExecutionSpace>>> {
        using type = typename T::template ExecutionViewType<ExecutionSpace>;
    };
  }  // namespace internal
  ...
  class Grid : public internal::GridBase<T, rank, Policies...> {
    public:
      ...
      /**
       * @brief Get a lightweight view of the grid for use in an inner loop running on `ExecutionSpace`
       *
       * If the storage policy does not define a dedicated `ExecutionViewType` for `ExecutionSpace`
       * (the common case for host-only storage policies), this falls back to returning a copy of the
       * grid itself. Since `Grid` has reference semantics, that copy still refers to the same
       * underlying data. Otherwise the call is forwarded to the storage policy's own
       * `getExecutionView()`, which returns a value type that is cheap to copy into e.g. a device
       * kernel.
       */
      template<typename ExecutionSpace = void>
      ExecutionViewType<ExecutionSpace> getExecutionView() {
        return this->template getExecutionViewImpl<ExecutionSpace, GridType, ExecutionViewType<ExecutionSpace>>(*this);
      }
