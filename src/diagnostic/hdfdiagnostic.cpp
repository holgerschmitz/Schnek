...
HdfIStream::HdfIStream() : HdfStream() {}

HdfIStream::HdfIStream(const char* fname) : HdfStream() {
  open(fname);
}

HdfIStream::HdfIStream(const HdfIStream& hdf) : HdfStream(hdf) {}
...
HdfOStream::HdfOStream() : HdfStream(), initialised(false) {}

HdfOStream::HdfOStream(const HdfOStream& hdf) : HdfStream(hdf), initialised(hdf.initialised) {}

HdfOStream::HdfOStream(const char* fname) : HdfStream(), initialised(false) {
  open(fname);
}
...
#else
  if (active) file_id = H5Fcreate(fname, H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);
