...
BOOST_FIXTURE_TEST_CASE( reduce_2d, GridTest )
{
...
BOOST_FIXTURE_TEST_CASE( reduce_gridparam_1d, GridTest )
{
  typedef schnek::Grid<double, 1, GridBoostTestCheck, GridStorage> GridType;
  GridType::IndexType lo, hi;
  boost::timer::progress_display show_progress(25);
  for (int n=0; n<5; ++n)
  {
    ...
    GridType g1(lo, hi);
    GridType g2(lo, hi);
    test_reduce_gridparam_1d(g1, g2);
    for (int m=0; m<5; ++m)
    {
      random_extent<1>(lo, hi);
      g1.resize(lo, hi);
...
BOOST_FIXTURE_TEST_CASE( mergegrids_1d, GridTest )
{
  typedef schnek::Grid<double, 1, GridBoostTestCheck, GridStorage> GridType;
  GridType::IndexType lo, hi;
  boost::timer::progress_display show_progress(25);
